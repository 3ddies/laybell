...
export async function deletePostById(postId: string): Promise<boolean> {
  ...
  try {
    ...
    let { data } = await supabase
      .from('posts')
      .select('media_url, thumbnail_url, cover_url, thumb_url, preview_url, slides, video_uid, legacy_media_url')
      .eq('id', postId)
      .single();
    if (!data) {
      ({ data } = await supabase
        .from('posts')
        .select('media_url, thumbnail_url, cover_url, thumb_url, preview_url, slides, video_uid')
        .eq('id', postId)
        .single() as any);
    }
