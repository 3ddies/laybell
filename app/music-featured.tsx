...
import { MAX_FEATURED, parseFeatured, saveFeatured } from '../lib/musicFeatured';
...
// Premium: choose up to four songs for the Featured rail at the top of the
// profile Music tab.
//
// Tap to add, tap again to remove. NUMBERED in pick order, because the rail is
// ordered and a plain checkmark would leave the artist unable to see — let alone
// choose — which song leads. Picking a fifth is refused rather than silently
// dropping the oldest: quietly undoing someone's earlier choice to honour their
// latest is the kind of helpfulness nobody asked for.
export default function MusicFeaturedScreen() {
  ...
  useEffect(() => {
    (async () => {
      ...
      setTracks(live);
      // Filtered against what still exists, so a song deleted since it was
      // picked does not occupy one of the four invisibly.
      const liveIds = new Set(live.map((p: any) => p.id));
      setPicked(parseFeatured((profRes.data as any)?.music_featured).filter((id) => liveIds.has(id)));
      setLoading(false);
    })();
  }, []);
  ...
  function toggle(id: string) {
    setPicked((prev) => {
      ...
      if (prev.length >= MAX_FEATURED) return prev; // full — the UI says so below
      return [...prev, id];
    });
  }
  ...
  async function save() {
    ...
    if (ok) {
      ...
      update?.({ music_featured: picked } as any);
      router.back();
    }
  }
    ...
    <SwipeBackPager>
      <View style={styles.container}>
        ...
        <View style={styles.intro}>
          ...
          <Text style={styles.sub}>{t('featured.sub', { max: MAX_FEATURED })}</Text>
          ...
          <ScrollView contentContainerStyle={[styles.list, { paddingBottom: SPACING.xxl + insets.bottom }]}>
            {tracks.map((tr) => {
              ...
              return (
                ...
                >
                  {tr.cover_url ? (
                    <Image source={{ uri: tr.cover_url }} style={styles.cover} contentFit="cover" cachePolicy="memory-disk" />
                  ) : (
                    <View style={[styles.cover, styles.coverEmpty]}>
                      <Ionicons name="musical-note" size={18} color={colors.textTertiary} />
                    </View>
                  )}
                  <Text style={styles.rowTitle} numberOfLines={1}>{tr.caption || t('album.untitled')}</Text>
                  {/* The NUMBER, not a tick: the rail is ordered, and the artist
                      has to be able to see which song leads it. */}
...
const makeStyles = (colors: ThemePalette) => StyleSheet.create({
  ...
  rowTitle: { flex: 1, color: colors.text, fontSize: 14.5, fontWeight: '600' },
  pos: {
