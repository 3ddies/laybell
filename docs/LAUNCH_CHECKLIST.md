...
**One-line status: 1.0.3 (11) is APPROVED on iOS (tag `v1.0.3-build11`). 1.0.5 is now in prep and SUPERSEDES 1.0.4 (build 12) — build 12 was uploaded to ASC on 2026-09-23 but never submitted, and the owner chose (2026-09-25) to bundle everything into one release instead. 1.0.5 = all of 1.0.4's content + React (reaction videos) + profile views + a story pulse. All pure JS + already-applied SQL — NO new native module, native fingerprint unchanged. Version bumped to 1.0.5 in `app.json`; `appVersionSource: remote` + `autoIncrement` means EAS builds it as build 13. Code committed + pushed on `dev` (`e0b4b88`). Pre-build gates all VERIFIED GREEN on 2026-09-25 (below). **iOS build 13 (1.0.5) BUILT + UPLOADED to App Store Connect on 2026-09-25** — build `4dccacf1-bf9d-4ea5-925d-db8c4bca15f4`; `eas submit` succeeded (submission `e38e3d80-def4-4d93-bad9-fd3456ba079b`, ASC API key on EAS), Apple now processing (~5–10 min). ⏳ **Owner finishes in ASC — create the 1.0.5 version, paste What's New (`docs/RELEASE_NOTES_1.0.5.md`), select build 13, Submit for Review** (`eas submit` only UPLOADED — Apple has NOT been asked to review yet, exactly like 1.0.4 build 12). ⚠️ **Re-confirm demo money OFF before clicking Submit.** Android: **1.0.5 (versionCode 11) BUILT 2026-09-25** (.aab ready, EAS `02b2b728-1f02-4960-81f6-4ac53fc0f3b0`); submit still blocked on the Play service-account key (`eas submit -p android`, draft) OR a manual `.aab` upload in the Play Console — the key is a credential the owner creates.**
