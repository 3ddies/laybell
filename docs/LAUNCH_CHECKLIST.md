...
**One-line status: 1.0.3 (11) is APPROVED on iOS (tag `v1.0.3-build11`). 1.0.5 is now in prep and SUPERSEDES 1.0.4 (build 12) — build 12 was uploaded to ASC on 2026-09-23 but never submitted, and the owner chose (2026-09-25) to bundle everything into one release instead. 1.0.5 = all of 1.0.4's content + React (reaction videos) + profile views + a story pulse. All pure JS + already-applied SQL — NO new native module, native fingerprint unchanged. Version bumped to 1.0.5 in `app.json`; `appVersionSource: remote` + `autoIncrement` means EAS builds it as build 13. Code committed + pushed on `dev` (`e0b4b88`). Pre-build gates all VERIFIED GREEN on 2026-09-25 (below). **iOS build 13 (1.0.5) BUILT on EAS 2026-09-25** — build `4dccacf1-bf9d-4ea5-925d-db8c4bca15f4`, .ipa ready (build-only, NOT submitted). Next: get build 13 into ASC (`eas submit` uploads it, or Transporter), then in App Store Connect create the 1.0.5 version, paste What's New (`docs/RELEASE_NOTES_1.0.5.md`), select build 13, Submit for Review — the owner clicks Submit. ⚠️ Re-confirm demo money OFF before submitting. Android: 1.0.3 (versionCode 10) built not submitted; still blocked on the Play service-account key.**
...
| Version | iOS | Android |
|---|---|---|
| 1.0.5 (build 13) | ✅ **BUILT on EAS 2026-09-25** (build `4dccacf1`, .ipa ready); React + profile views bundled onto 1.0.4, gates green; **owner: `eas submit` → create version/What's New/select build 13/Submit for Review** | ⏳ not built |
