...
| 10. Money test | 🟡 **iOS-only sitting** — owner has no Android device for a while (2026-08-09). Server-side money code (webhook → ledger → tips/shop/offers RPCs) is store-agnostic, so the iOS sitting exercises all of it; the untested remainder is the thin Android client layer (RC Play SDK + Play Billing). **Before promoting Android beyond internal testing, run the 15-min Android purchase test on a real device.** ASC-side same day: all 7 iOS products metadata-completed (screenshots 1242×2208 + review notes), plus sub ranked Level 1 above Premium, sandbox tester created. |
