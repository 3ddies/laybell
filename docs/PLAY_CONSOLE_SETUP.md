...
| 5. Upload to internal testing | ✅ **Done 2026-08-09** — key-carrying build ed3a2623 uploaded, version 4 (1.0.0), published to internal testing; app-signing SHA-256 wired into assetlinks.json, SHA-1 into a Google OAuth Android client |
| 6. Create the products | ✅ **Done 2026-08-09** — 5 one-time credit products (new "purchase option" model, option id `buy`) + `laybell_premium_monthly:monthly` + `laybell_premium_plus_1999:monthly` (base plan id `monthly` on both), all activated |
| 7. RevenueCat wiring | ✅ **Done 2026-08-09** — all 7 Play products imported; entitlements `premium` (both stores' monthly) + `premium_plus` (both stores' plus, iOS product created same day); offerings: `default` (+ new `plus_monthly` package) and new `credits` offering (5 packages, both store slots). ⚠️ RC's iOS catalog has a stale `laybell_credits_49.99` (DOTTED) entry from July — ASC's real id is `laybell_credits_4999` (verified); a correct RC entry now exists and is what the packages use. Never attach the dotted one. |
| 8. androidApiKey + REBUILD | ✅ **Done** — key in `a1241aa`, build ed3a2623 carries it |
| 9. License tester | ✅ **Done 2026-08-09** — email list "Laybell testers" (ehalldc@gmail.com + 3ddiehall@gmail.com), RESPOND_NORMALLY |
| 10. Money test | ✅ **COMPLETE 2026-08-09 — FULL RESULTS: `docs/MONEY_TEST_2026-08-09.md`** (12 paths + 7 guards + a 557-object schema audit). Summary below. **(iOS device + server-side RPCs).** Sandbox purchase $4.99 → webhook → ledger double-entry verified (buyer was the phone's signed-in account, shpwkvr7jg). Tip $6.00 via real tip_with_credits: fee 180¢ (30%) + payout 420¢ w/ 14-day hold. Shop sell $2.99 via shop_buy_with_credits: instant delivery, fee 90¢, seller 209¢ held, listing → sold w/ sales_count 1. Offer $1.99: escrow hold → seller decline → refund leg mirrors exactly (ledger legs 38s apart). `ledger_verify()` = **0 rows** over everything. **Guards that fired correctly under attack**: tip minimum ($6 floor), sold-listing refusal, insufficient-funds refusal, has_purchases takedown block (even against a direct superuser UPDATE), plus full-transaction rollbacks on every failure. **Deferred**: Android device purchase (before Android leaves internal testing) · Stripe Express form completion (owner deferred; Laybell's side — link mint, in-app sheet, return page — proven) · live-UI tip via the donate modal (needs 2 devices) · optional second-product device purchase. Test artifacts: sold "Money-test beat" listing kept (takedown guard, honest history), stream/donation rows retained (append-only ledger); fresh-start reset clears pre-launch. |
